...
const Deposit=async (req,res)=>{  
  ...
  try {
      ...
      if(updatereg.length<=0) return res.status(400).send('Need to Login')             
          let bal=Number(updatereg[0].balance)
          const value=Number(req.body.deposit);
          const add=value+bal;
          console.log(value);
              const update=await  User.findOneAndUpdate({email:req.user.email},{$set:{deposit:add,balance:add}},{new:true})
...
export {register,login,Deposit,withdraw,getAllUser,myprofile}